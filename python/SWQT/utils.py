...
def regularize_output_approx(approx, image_width, image_height, bVisualized, toleratedMinDistCoeff=0.1):
    ...
    if len(approx) == 4:
        ...
        for i in range(3):
            approxFirstPointDist.append(_distance_2D(approx[0], approx[i+1]))
        ...
        triAngleCos = []
        for i in range(3):
            triAngleCos.append(((approx[i][0]-approx[(i+2)%3][0])*(approx[i][0]-approx[(i+1)%3][0])+(approx[i][1]-approx[(i+2)%3][1])*(approx[i][1]-approx[(i+1)%3][1]))/(_distance_2D(approx[i], approx[(i+2)%3])*_distance_2D(approx[i], approx[(i+1)%3])))
        cosMinIndex = triAngleCos.index(min(triAngleCos))
        res.append(approx[cosMinIndex])
        res.append(approx[(cosMinIndex+1)%3])
        caledForthPointX = approx[(cosMinIndex+2)%3][0]+approx[(cosMinIndex+1)%3][0]-approx[cosMinIndex][0]
        caledForthPointY = approx[(cosMinIndex+2)%3][1]+approx[(cosMinIndex+1)%3][1]-approx[cosMinIndex][1]
        ...
        for i in range(len(approx)):
            disTo0.append(math.sqrt(approx[i][0]**2+approx[i][1]**2))
            k.append(approx[i][1]/approx[i][0])
...
def _find_polys(image, bVisualized, minAreaCoeff=0.02, approxPolyDPCoeff=0.05):
    ...
    if bVisualized:
        contoursShow = np.zeros((image.shape[0], image.shape[1], 3), dtype=int)
    ...
    for idx, contour in enumerate(contours):
        ...
        approx = cv2.approxPolyDP(contour, len*approxPolyDPCoeff, False)
        if bVisualized:
            contourShow = np.zeros((image.shape[0], image.shape[1], 3), dtype=int)
            ...
            for approx_point in approx:
                print('(%d, %d), '%(approx_point[0], approx_point[1]), end='')
            print()
            cv2.waitKey()
        ...
        if bVisualized:
            ...
            for i in range(4):
                print('(%d, %d), '%(reg[i][0], reg[i][1]), end='')
            print()
        polys.append(reg)
        rect = cv2.boundingRect(reg)
        if bVisualized:
            c = np.zeros((image.shape[0], image.shape[1], 3), dtype=int)
            for pt in contour:
                c[pt[1]][pt[0]] = [255, 0, 0]
            for pt in reg:
                cv2.circle(c, pt, 5, (255, 255, 0))
...
def visualize_disparity(disp):
    ...
    maxV = np.max(disp)
    scale = 255/(maxV-minV)
    disp = (disp-minV)*scale
    return disp

def reproject(disparity, q, bmm, zclamp=[100, 1000]):
    image3d = cv2.reprojectImageTo3D(disparity, q, True, -1).reshape(disparity.shape[0], disparity.shape[1], 3)
    if not bmm:
        zclamp[0] /= 1000
        zclamp[1] /= 1000
